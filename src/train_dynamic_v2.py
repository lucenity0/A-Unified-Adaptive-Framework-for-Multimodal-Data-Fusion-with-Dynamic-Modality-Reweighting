...
train_dynamic_v2.py
===================
Enhanced training for Dynamic Reweighting - targeting 75% accuracy.
...
CONFIG = {
    ...
    "checkpoint_name": "best_model_dynamic_v2.pt",
    "patience"       : 7
}
...
class AdaptiveFusionModelV2(nn.Module):
    """Enhanced model for ViT-L/14 CLIP."""
    def __init__(self, clip_model_name="openai/clip-vit-large-patch14", unfreeze_layers=4):
        super().__init__()
        ...
        self.gating = DynamicGatingNetworkV2(embed_dim=self.embed_dim)
...
# ─────────────────────────────────────────────────────────────────
def train_one_epoch(
    model, loader, optimizer, scheduler, criterion, device, grad_accum, entropy_weight, alpha_fairness_weight
):
    model.train()
    total_loss = 0
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        alpha_group_var = torch.tensor(0.0, device=device)
        ...
        if unique_groups.numel() > 1:
            ...
            for g in unique_groups:
                mask = groups == g
                if mask.any():
                    group_means.append(alpha[mask].mean())
            if len(group_means) > 1:
                alpha_group_var = torch.stack(group_means).var(unbiased=False)

        loss = loss + entropy_weight * entropy_loss + alpha_fairness_weight * alpha_group_var
...
def main():
    ...
    for epoch in range(CONFIG["num_epochs"]):
        print(f"\n{'='*60}")
        print(f"EPOCH {epoch+1}/{CONFIG['num_epochs']} -- Dynamic Reweighting V2")
        print(f"{'='*60}")
        
        train_loss, train_auroc = train_one_epoch(
            model, train_loader, optimizer, scheduler, criterion, device,
            CONFIG["grad_accum"], CONFIG["entropy_weight"], CONFIG["alpha_fairness_weight"]
        )
